...
export default function Home() {
  ...
  const handleAudioSync = useCallback(async (params: { assetA: string; assetB: string; sampleRate: number; correlationSampleSize: number; initialGranularity: number }) => {
  ...
  const handleApplyAudioSync = useCallback((result: { analyzedClipIds: [string, string]; analyzedTabId: string; anchorClipId: string; nonAnchorClipId: string; nonAnchorIsAssetB: boolean; anchorInPoint: number; nonAnchorInPoint: number; offsetSeconds: number }) => {
    ...
    console.log('[AudioSync Apply]', {
      anchorClipId, nonAnchorClipId, nonAnchorIsAssetB,
      offsetSeconds, assetOffset, rawStart,
      anchorStart: anchorClip.start, anchorInPoint, nonAnchorInPoint,
      analyzedTabId,
    });
