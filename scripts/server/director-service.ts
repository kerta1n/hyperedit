...
import { generateWithLLM, parseLLMJson } from './llm-gateway.ts';

// Director edit-command generation (top-level POST /ai-edit, exported for the
// entry's dispatch — not session-scoped). Moved as-is per the virtual-first
// decree: R2/R3 replaces this whole mechanism with semantic operations, so no
// investment beyond relocation.
...
export async function handleAiEditCommand(req: IncomingMessage, res: ServerResponse) {
  try {
