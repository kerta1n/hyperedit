...
// System prompt for the Director's edit-command generation (moved server-side
// from the retired Cloudflare worker).
const EDIT_COMMAND_SYSTEM_PROMPT = `You are a video editing AI assistant that helps users edit their videos using FFmpeg commands.
...
// Director edit-command generation: prompt -> { command, explanation }.
// Single synchronous request — the server is one long-lived process, so no
// job map / status polling is needed (the worker's pendingRequests pattern
// was broken across isolates anyway).
async function handleAiEditCommand(req, res) {
  try {
    ...
    const text = await generateWithLLM(prompt, {
      systemPrompt: EDIT_COMMAND_SYSTEM_PROMPT,
...
const server = http.createServer(async (req, res) => {
  ...
  // Director edit-command generation (repointed from the Cloudflare worker)
  if (req.method === 'POST' && path === '/ai-edit') {
    await handleAiEditCommand(req, res);
    return;
  }
